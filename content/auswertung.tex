...
\begin{equation}
    \frac{\symup{d}T}{\symup{dt}} \approx F(t) = At^3 + Bt^2 + Ct + D
\end{equation}
gewählt.
\begin{figure}[H]
  \centering
...
\end{equation}
In folgendem halblogarithmischen Diagramm, ist eine Ausgleichsgerade eingefügt aus dessen Steigung sich L berechnen lässt.
\begin{figure}[H]
    \centering
...
%
Daraus folgt ein Massendurchsatz von:
\begin{table}[H]
    \centering
...
Die Dichte von $\ce{Cl2F2C}$ bei $T=\SI{0}{\celsius}$ und $p=\SI{1}{\bar}$ liegt bei \mbox{$\rho_0=\SI{5.51}{\gram\per\liter}$ \cite{Cl2F2C}} und
